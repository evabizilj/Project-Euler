...
containing one-hundred rows; it cannot be solved by brute 
...
int main()
{
    ...
    for (int i = 1; i < 15; i++)
    {
        sum[i][0] = array[i][0] + sum[i - 1][0];
        sum[i][i] = array[i][i] + sum[i - 1][i - 1];
        for (int j = 1; j < i; j++)
            sum[i][j] = array[i][j] + fmax(sum[i - 1][j - 1], sum[i - 1][j]);
    }
